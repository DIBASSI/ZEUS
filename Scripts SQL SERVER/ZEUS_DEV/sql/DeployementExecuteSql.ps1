...
foreach ($filename in get-childitem -path $FolderPath -filter "*.sql")
{
	...
	invoke-sqlcmd –ServerInstance $SQLServer -Database $DatabaseName -Variable $sqlParameters -InputFile $filename.fullname -AbortOnError -outputsqlerrors $true -Verbose -ErrorAction Stop  
